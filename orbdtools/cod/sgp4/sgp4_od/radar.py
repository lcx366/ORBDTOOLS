...
def fun_resi_radar(ele_bstar,ta0,t,posnp_nd):
    """
    Residual function for least squares estimation of orbit elements and bstar using radar range+angle measurements with SGP4 propagator.

    Usage:
        >>> residuals = fun_resi_radar(ele_bstar,ta0,t,posnp_nd)
    Inputs:
        ...
        posnp_nd -> [2D array with shape of nx3] Cartesian coordinates of the space objects in GCRF(Geocentric Celestial Reference Frame) in in [Length unit], 
    ...
    residuals = norm(rs_vec - posnp_nd,axis=1)

    return residuals

def sgp4_od_radar(ele0,ta0,ta,xyz_site,posnp,params):
    """
    Cataloging orbit determination using radar range+angle measurements with SGP4 propagator. 
    ...
    Inputs:
        ...
        posnp -> [2D array with shape of nx3] Cartesian coordinates of the space objects in GCRF, [km]
    ...
    res = least_squares(fun_resi_radar,ele_bstar, args=(ta0,t,posnp_nd),loss='huber',bounds=([0,0,0,0,0,0,-1],[1,1,np.pi,twopi,twopi,twopi,1]))
